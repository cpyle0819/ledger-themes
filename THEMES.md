...
This guide takes you from an empty folder to an installed, switchable theme. Read
The Ledger's `CLAUDE.md` for the theming architecture; read
[`packages/the-ledger/theme.css`](packages/the-ledger/theme.css) as the annotated
reference — it is the token contract and the worked example at once.
...
Copy an existing package as your starting point. `space-opera` is the better
template than the reference theme: it inverts the card surface from light
parchment to a dark data-plate, so it exercises the full contract — text-token
flips, recipe overrides, a distinct logo — without the reference theme's baked-in
texture assets.

```
cp -r packages/space-opera packages/<your-theme>
```

Then edit `package.json`: set `name` (the bundled themes use
`@cpyle0819/ledger-theme-<id>`; a third-party theme uses its own scope or an
unscoped `ledger-theme-<id>` — The Ledger discovers any package with a
`ledgerTheme` block, the name is yours), write a one-line `description` of the
look, and fill the `ledgerTheme` block.
...
- **`::-webkit-scrollbar*`** — scrollbar thumb/track for light-DOM scrollers.
  Shadow-DOM scrollers pick up the matching constructable sheet from The Ledger's
  `shared-styles.ts`, so styling these covers the visible rest.
...
blend mode over a base gradient, as `the-ledger`'s `ledger-board` rule does.

## Step 4 — ship a logo (optional)

The logo is a self-registering custom element served from your package, declared
in the manifest as `{tag, src}`. Model it on
[`packages/the-ledger/mark.js`](packages/the-ledger/mark.js):
...
From the monorepo root, `npm install` links the new workspace package. To load it
in a running Ledger deployment, install it there — discovery is automatic, no code
change:
...
Each theme publishes independently; scoped packages set
`publishConfig.access: "public"` so they land public.

```
npm publish -w <your-theme-package>
```

Within this monorepo, publish every package at once with `npm run publish:all`.
Bump the package
